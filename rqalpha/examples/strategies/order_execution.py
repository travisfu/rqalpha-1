...
def handle_bar(context, bar_dict):
    ...
    file_path = "E:/test/" + context.now.strftime('%Y%m%d') + ".xlsx"
    print(file_path)
    try:
    ...
    for future_order in portfolio_future.index:
        ...
        # 目标手数
        target_amount = portfolio_future.loc[future_order, 'target_amount']
        # 净持仓量
        ...
        if net_exe_amount < 0:
            if future_positions[future_order].buy_quantity > 0:
                if net_exe_amount <= future_positions[future_order].buy_quantity:
                    sell_close(future_order, abs(net_exe_amount))
                else:
                    sell_close(future_order, future_positions[future_order].sell_quantity)
                    sell_open(future_order, abs(net_exe_amount) - future_positions[future_order].sell_quantity)
            else:
